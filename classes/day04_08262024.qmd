...
$$CI = \hat{\beta} \pm t_{\alpha/2, \text{rdf}}\ \text{se},$$
where $t_{\alpha/2, \text{rdf}}$ is the t-score that corresponds to the confidence level (i.e., $\alpha$) with the residuals degrees of freedom $\text{rdf}$, and $\text{se}$ is the standard error. 
