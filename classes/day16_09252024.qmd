...
for $i =1, 2, ..., n$ where $n$ is the total number of observations, $y_i$ is the $i$th observation of grain yield, that arises from a Normal distribution with mean $\mu_i$ and variance $\sigma^2$. The $mu_i$ is defined as a function of the parameters $\beta_0$, $beta_1$ and $\beta_2$, and the predictor $x_i$, plant density.
...
dd %>% 
  ggplot(aes(plant_density, epsilon.hat))+
  geom_point()+
  labs(x = expression(Plant~Density~(plants~m^{-2})), 
       y = expression(Grain~Yield~(Mg~ha^{-1})))+
  theme_classic()+
...
One available testing method to test linearity is looking at the estimated parameters.   
```{r}
summary(m1)
```
