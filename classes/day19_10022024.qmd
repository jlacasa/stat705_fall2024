...
The AIC of a given model $M$ and observed data $\mathbf{y}$ is calculated as $$AIC_M = 2k - 2\log(\hat{L}),$$ where $k$ is the penalty per parameter (defaults to 2 in `R`), $p$ is the number of parameters in the model and $\hat{L}$ is the maximized value of the likelihood function for the model (i.e., $\hat{L}=p(\mathbf{y}|\hat{\boldsymbol\beta}, M)$).

#### Bayesian Information Criterion (BIC)

The BIC of a given model (and observed data) is calculated as $$BIC = k\log(p) - 2\log(\hat{L}),$$ where $k$ is the penalty per parameter (defaults to 2 in `R`), $p$ is the number of parameters in the model, and $\hat{L}$ is the maximized value of the likelihood function for the model (i.e., $\hat{L}=p(\mathbf{y}|\hat{\boldsymbol\beta}, M)$).
