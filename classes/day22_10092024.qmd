...
```{r}
cv::cv(m1, seed=1234, confint=TRUE)
cv::cv(m2, k = "loo")
cv::cv(m3)
plot(cv::cv(m4))
...
metrics <- data.frame(model = c("N_total + I(N_total^2)",
                      ...
                      `Rsquared.kCV` = c(kcv_m1$results$Rsquared, NA, NA,
                                       kcv_m4$results$Rsquared, kcv_m5$results$Rsquared, kcv_m6$results$Rsquared), 
                      `RsquaredSD.kCV` = c(kcv_m1$results$RsquaredSD, NA, NA,
                      ...
                      `Rsquared.looCV` = c(loocv_m1$results$Rsquared, NA, NA,
                                          loocv_m4$results$Rsquared, loocv_m5$results$Rsquared, loocv_m6$results$Rsquared) ) %>%
  mutate(across(R2:`Rsquared.looCV`, ~round(., 3)))
